...
resource "azurerm_linux_virtual_machine" "web_linux_vm" {
  ...
  computer_name         = "web-linux-${each.key}"
  resource_group_name   = azurerm_resource_group.rg.name
  location              = azurerm_resource_group.rg.location
  size                  = "Standard_DS1_v2"
  admin_username        = "azureuser"
  # network_interface_ids = [element(azurerm_network_interface.web_linux_vm_nic[*].id, count.index)]
  network_interface_ids = ["${azurerm_network_interface.web_linux_vm_nic[each.key].id}"]
  ...
  os_disk {
    ...
    name = "${local.resource_name_prefix}-web-linux-${each.key}-os-disk"
    caching              = "ReadWrite"
